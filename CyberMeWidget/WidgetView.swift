...
struct CyberMeWidgetEntryView: View {
  ...
  var updateStr1: String {
    ...
    let updateAt = Date(
      timeIntervalSince1970: TimeInterval(entry.dashboard.updateAt))
    let now = Date()
  ...
  var magicNumber: String {
    let base =
      (store!.dictionary(forKey: "settings") as? [String: String] ?? [:])[
        "wireguardBasePort"]
  ...
  var largeView: some View {
    ...
    let bg = WidgetBackground(
      rawValue: store!.string(forKey: "widgetBG") ?? "mountain")
    let alertOn = store!.bool(forKey: "alert")
    let fakeTodo = data.tickets.filter(\.isUncomming).map { ticket in
      Todo(
        title: ticket.description, isFinished: false, create_at: "0",
        list: "Work")
    }
    ...
    let bodyMassStr =
      bodyMass == "0.0" ? "" : "\(bodyMassNum >= 0 ? "▼" : "▲")\(bodyMass)kg"

    //let mind = data.fitnessInfo?.mindful ?? 0.0
    let mindStr = ""  //mind != 0.0 ? "🫧" : "⚠︎M"
    let fitInfo = "\(bodyMassStr) \(mindStr)"

    let showMark = data.limit?.progress?.todayOk ?? false
    let percent = data.limit?.progress?.percent ?? 0
    let haveLimit = data.limit?.plan?.id != nil
    let limitStr = "\(showMark ? "✔︎ " : "")\(String(format: "%.0f%%", percent * 100))"
      ...
      HStack {
        ...
        if data.needDiaryReport && haveLimit {
          VStack {
            Text("今日日报")
              .foregroundColor(
                data.needDiaryReport ? .white : Color("BackgroundColor-Heavy")
              )
              .font(.system(size: basic - 3))
            Text(limitStr)
              .foregroundColor(
                needFitness ? .white : Color("BackgroundColor-Heavy")
              )
              .font(.system(size: basic - 3))
          }
        } else if data.needDiaryReport && needFitness {
          VStack {
            Text("今日日报")
              .foregroundColor(
                data.needDiaryReport ? .white : Color("BackgroundColor-Heavy")
        ...
        if !data.todo.isEmpty {
          VStack(alignment: .leading, spacing: 2) {
            ForEach(data.todo.prefix(3), id: \.self) { item in
              if !fakeTodo.isEmpty
                && fakeTodo.contains(where: { t in t.title == item.title })
              {
  ...
  var smallView: some View {
    ...
    let update = Date(
      timeIntervalSince1970: Double(data.car?.reportTime ?? 0) / 1000)
    ...
    if let car = data.car {
      ...
      if status.parkingBrake == "active"
        && (status.tyre != "checked" || status.lock != "locked"
          || status.doors != "closed" || status.windows != "closed")
      {
        warning = true
      }
    }
          ...
          HStack {
            if warning {
              Image("vw")
                .resizable()
                .colorMultiply(
                  Color(red: 0.7, green: 0.2, blue: 0.2, opacity: 0.9)
                )
  ...
  var todoView: some View {
    ...
    let fakeTodo = data.tickets.filter(\.isUncomming).map { ticket in
      Todo(
        title: ticket.description, isFinished: false, create_at: "0", list: "车票"
      )
    }
  ...
  var workView: some View {
    ...
    let (temp, isYesterday) = data.tempSmartInfo
    let tempHighDetail =
      temp != nil && temp!.diffHigh != nil && Int(temp!.diffHigh!) != 0
    let tempLowDetail =
      temp != nil && temp!.diffLow != nil && Int(temp!.diffLow!) != 0
    var tempSum = "-"
    if let temp = temp {
      let highTemp =
        "\(Int(temp.high))\(tempHighDetail ? String(format: "%+.0f", temp.diffHigh!) : "")"
      let lowTemp =
        "\(Int(temp.low))\(tempLowDetail ? String(format: "%+.0f", temp.diffLow!) : "")"
  ...
  var weatherView: some View {
    ...
    let (temp, isYesterday) = data.tempSmartInfo
    var weatherInfo =
      (data.weatherInfo == nil || data.weatherInfo!.isEmpty)
      ? "没有天气信息" : data.weatherInfo!
    let tempHighDetail =
      temp != nil && temp!.diffHigh != nil && Int(temp!.diffHigh!) != 0
    let tempLowDetail =
      temp != nil && temp!.diffLow != nil && Int(temp!.diffLow!) != 0

    let useWeather = UserDefaults(suiteName: Default.groupName)!.bool(
      forKey: "useWeather")
    if !useWeather {
      let bodyMassNum = data.fitnessInfo?.bodyMassDay30 ?? 0.0
      ...
      bodyMass = bodyMass == "0.0" ? "0" : bodyMass
      let bodyMassStr =
        bodyMass == "0.0" ? "" : "\(bodyMassNum <= 0 ? "▼" : "▲")\(bodyMass)"
      ...
      if let temp = temp {
        ...
        let lowTemp =
          "↓\(Int(temp.low))\(tempLowDetail ? String(format: "%+.0f", temp.diffLow!) : "")"
        return Text(
          "\(isYesterday ? "*" : "")\(highTemp)\(lowTemp) \(weatherInfo)")
      }
    }
